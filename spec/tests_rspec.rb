...
RSpec.describe Syntax do
  describe '#camelcase' do
    it 'gives camelcase syntax issues' do
      expect(syntax.camelcase("var my_favorite_color = \"#112C85\";",0)).to eql("use camelCase for variable names at line 0 \n")
    end
  end

  describe '#prefer_exponentiation_operator' do
    it 'gives Math.pow syntax issues' do
      expect(syntax.prefer_exponentiation_operator("const foo = Math.pow(2, 8);",0)).to eql("use ** instead of Math.pow at line 0 \n")
    end
  end
